...
class RabinKarpSearch(SearchStrategy):
    ...
    def search(
        ...
        for i in range(m):
            pattern_hash = (
                self.alphabet_size * pattern_hash + ord(pattern[i])
            ) % self.prime
        ...
        for shift in range(n - m + 1):
            if step_by_step:
                append_log(
                    logs,
                    f"shift={shift}, hash_text={window_hash}, hash_pattern={pattern_hash}",
                )

            if pattern_hash == window_hash:
                if step_by_step:
                    append_log(
                        logs, "hash igual: iniciando verificacao caractere a caractere"
                    )
            ...
            if shift < n - m:
                window_hash = (
                    self.alphabet_size * (window_hash - ord(text[shift]) * h)
                    + ord(text[shift + m])
                ) % self.prime
