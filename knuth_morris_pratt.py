...
class KMPSearch(SearchStrategy):
    ...
    def _build_lps(
        self, pattern: str, step_by_step: bool, logs: list[str]
    ) -> tuple[list[int], int]:
        m = len(pattern)
        lps = [0] * m
        length = 0
        ...
        while i < m:
            ...
            if step_by_step:
                append_log(
                    logs,
                    f"LPS: pattern[{i}]='{pattern[i]}' vs pattern[{length}]='{pattern[length]}'",
                )
    ...
    def search(
        ...
        while i < n:
            ...
            if step_by_step:
                append_log(
                    logs, f"Busca: text[{i}]='{text[i]}' vs pattern[{j}]='{pattern[j]}'"
                )
